...
import { EyeIcon, PencilIcon, Rocket, Save } from "lucide-react";
import { useForm } from "react-hook-form";
import * as z from "zod";

import { IBlogDetail, IBlogForm } from "@/types/blog";
...
import { BlogFormSchema, BlogFormSchemaType } from "@/types/blog";
        ...
        <div className="flex flex-wrap items-center gap-2 border-b p-5 sm:flex-row sm:justify-between">
          <div className="flex flex-wrap items-center gap-5">
            <span
              ...
              role="button"
              tabIndex={0}
              className="flex items-center gap-2 rounded-md border px-2 py-1.5 text-sm transition-all hover:border-zinc-400"
            >
              {!isPreview ? (
                      ...
                      {isPreview ? (
                        <div className="relative mt-10 h-80 w-full rounded-md border">
                          {form.getValues().image_url ? (
                            ...
                            <span className="text-gray-400 flex justify-center items-center inset-0 absolute">
...
const ImageEror = ({ src }: { src: string }) => {
  if (!src) {
    return <h1>Invalid</h1>; // Menampilkan pesan error jika src tidak valid
  }

  return <Image src={src} alt="Image" width={100} height={100} />;
};
