...
import {deleteLayananIbuHamil} from "./action";
...
export function DataTableRowActions({
  ...
  const handleDeleteClick = async () => {
    setIsDeleting(true);
    try {
      const result = await deleteLayananIbuHamil(id_layanan);

      if (result.success) {
        toast({
          title: "Layanan berhasil dihapus.",
          description: "Data layanan Ibu Hamil berhasil dihapus.",
        });
        window.location.reload();
      } else {
        throw new Error("Gagal menghapus layanan Ibu Hamil");
      }
    } catch (error) {
      toast({
